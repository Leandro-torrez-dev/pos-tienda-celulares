...
export function App() {
  ...
  const handleLogout = () => {
    ...
    setToken(null);
    setCart([]);
    setCurrentView('catalogo'); // Reseteamos la vista por seguridad
  ...
  // Función para abrir la pantalla de detalles de un producto
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-6 relative">
        ...
        <div className="flex gap-2">
          {currentView === 'detalle' && (
            <button
              ...
              className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-4 py-2 rounded-lg text-xs transition-colors border border-gray-200"
            >
              ⬅ Volver al Catálogo
            </button>
          )}
      ...
      {/* 🖼️ VISTA 1: PANTALLA DEL CATÁLOGO GENERAL */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition-shadow">
                    {/* Al hacer clic en la tarjeta o el título, abrimos los detalles */}
                    <div className="flex gap-4 cursor-pointer" onClick={() => viewProductDetails(product)}>
                      ...
                      <div className="flex flex-col justify-between flex-1">
                        <div>
                          ...
                          <h2 className="text-xs font-bold text-gray-800 line-clamp-2 mt-0.5 hover:text-blue-600 transition-colors">{product.title}</h2>
                        </div>
      ...
      {/* 🔍 VISTA 2: INTERFAZ DEL DETALLE INDIVIDUAL DE PRODUCTO */}
      {currentView === 'detalle' && selectedProduct && (
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200 p-6 md:p-8 animate-slide-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            
            {/* Contenedor de la Imagen */}
            <div className="w-full h-72 md:h-96 bg-gray-50 rounded-xl p-6 flex items-center justify-center border border-gray-100">
            ...
            <div className="flex flex-col justify-between h-full space-y-4">
              <div>
                <span className="bg-blue-100 text-blue-800 text-xs font-extrabold px-2.5 py-1 rounded-full uppercase tracking-wider">
                  {categoryTranslations[selectedProduct.category] || selectedProduct.category}
                </span>
                <h2 className="text-xl md:text-2xl font-black text-gray-800 mt-3 tracking-tight">{selectedProduct.title}</h2>
                <div className="flex items-center gap-2 mt-2">
                  <span className="text-amber-500 font-bold text-sm">⭐ {selectedProduct.rating?.rate || '4.5'}</span>
                  <span className="text-gray-400 text-xs">({selectedProduct.rating?.count || '120'} reseñas del sistema)</span>
                </div>
              </div>

              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400">Descripción del Producto</h3>
                <p className="text-sm text-gray-600 mt-1 leading-relaxed text-justify">{selectedProduct.description}</p>
              </div>

              <div className="border-t border-gray-100 pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <span className="text-xs font-semibold text-gray-400 block">Precio Unitario:</span>
                  <span className="text-3xl font-black text-gray-950">${selectedProduct.price.toFixed(2)}</span>
                </div>
                
                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    onClick={() => addToCart(selectedProduct)}
                    className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl text-xs transition-all shadow-md hover:shadow-lg"
                  >
                    🛒 Añadir al Carrito POS
                  </button>
                  <button
                    onClick={() => setCurrentView('catalogo')}
                    className="bg-gray-100 hover:bg-gray-200 text-gray-600 font-medium px-4 py-3 rounded-xl text-xs transition-colors border border-gray-200"
                  >
                    Volver
                  </button>
                </div>
